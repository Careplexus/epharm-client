...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <section className="w-full max-w-[240px] bg-white shadow rounded-md overflow-hidden hover:shadow-lg transition duration-200">
      ...
      {/* Product Image */}
      <header className="bg-white pt-3 px-2">
      ...
      <main className="pt-4 px-3 pb-2 space-y-1">
        <Link to={`/product/${product.id}`} className="block text-gray-800 hover:text-primary transition-colors duration-200">
          <h2 className="font-semibold text-sm line-clamp-2">{product.name}</h2>
        </Link>
        <div className="flex justify-between items-center">
          <p className="text-green-700 font-bold text-sm">&#8358;{numericPrice.toLocaleString()}</p>
